# [Stage III: GS-DPO] GRPO training entrypoint, invoked by finetune_stage3_gs_dpo.sh.
# Loads the reward decomposition from reward_funcs.py via load_reward_funcs().
#
# Depends on the surrounding Qwen-VL-Series-Finetune framework — drop into src/train/.
...
class EpochTimeBudgetCallback(TrainerCallback):
    """Stops training after whichever epoch finishes last before the next one
    would blow past MAX_TRAIN_SECONDS, so the job exits cleanly (final save,
    ready for eval) instead of being killed mid-epoch by the Slurm wall-clock
    limit. Estimates next-epoch cost from the epoch that just finished."""
    ...
    def on_train_begin(self, args, state, control, **kwargs):
        ...
        self.last_epoch_end = self.start_time
        return control

    def on_epoch_end(self, args, state, control, **kwargs):
        now = time.time()
        epoch_duration = now - self.last_epoch_end
        self.last_epoch_end = now
        elapsed = now - self.start_time
        if elapsed + epoch_duration > self.max_seconds:
            rank0_print(
                f"[EpochTimeBudgetCallback] elapsed={elapsed:.0f}s + last "
                f"epoch={epoch_duration:.0f}s would exceed budget "
                f"{self.max_seconds:.0f}s; stopping training early at "
...
def train():
    ...
    max_train_seconds = os.environ.get("MAX_TRAIN_SECONDS")
    if max_train_seconds:
        trainer.add_callback(EpochTimeBudgetCallback(float(max_train_seconds)))

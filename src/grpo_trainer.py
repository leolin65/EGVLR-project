...
class QwenGRPOTrainer(GRPOTrainer):
    ...
    def _generate_and_score_completions(
        ...
        forward_kwargs, num_images = self._build_scoring_forward_kwargs(
            prompts, images, videos, video_kwargs, completion_ids, prompt_mm_token_type_ids
        )
    ...
    def _build_scoring_forward_kwargs(self, prompts, images, videos, video_kwargs, completion_ids, prompt_mm_token_type_ids):
        """Builds (forward_kwargs, num_images) for the policy/reference scoring
        pass over the full prompt+completion sequence (pixel_values,
        image_grid_thw, mm_token_type_ids, etc).

        Default (single-turn) implementation: images only ever appear in the
        PROMPT -- a completion never introduces a new image -- so
        mm_token_type_ids for the completion portion is simply zero-filled
        and num_images is just the per-sample prompt-side image count.
        Extracted out of `_generate_and_score_completions` as a pure refactor
        (no behavior change here) so that a subclass can override just this
        piece for a multi-turn setting where completions introduce new
        images mid-generation, which this default implementation's
        "completion is always text-only" assumption does not support.
        """
        ...
        return forward_kwargs, num_images

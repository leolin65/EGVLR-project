# [Stage II: KG-IT] One-normal visual QA converter.
# Originally: Stage2_Dataset/read_mmad.py (NOT the same script as
# stage1_pve_ft/build_synthetic_pve_ft_qa.py, despite the old shared name
# 'read_mmad.py' in both folders — this one does format conversion only,
# no image synthesis).
# Converts the paired query/normal-reference MMAD QA source
# (mmad_train5_with_think_dk_verified_flat.jsonl) into the final QA json.
# NOTE: its default output filename upstream was 'test_stage2.json' —
# despite the name, this is a *training* source (KG-IT's one-normal visual
# QA), not a held-out eval split. Renamed here to avoid that confusion.
...
python convert_one_normal_visual_qa.py \
  --input_json /home/psmc2/project/dataset/Stage2_Dataset/mmad_train5_with_think_dk_verified_flat.jsonl \
  --src_root /home/psmc2/project/dataset/MMAD \
  --dst_root /home/psmc2/project/dataset/Stage2_Dataset \
  --output_json /home/psmc2/project/dataset/Stage2_Dataset/test_stage2.json \
  --long_edge 256 \
  --skip_missing
"""
